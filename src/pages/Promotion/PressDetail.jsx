...
const isSameSiteArticle = (data, site) => !data.siteId || data.siteId === site;
...
export default function PressDetail() {
    ...
    const fetchArticle = async () => {
      try {
        ...
        if (snap.exists()) {
          ...
          if (isSameSiteArticle(data, site)) {
            setArticle({
              id: snap.id,
        ...
        const slugDoc = slugSnap.docs.find((item) =>
          isSameSiteArticle(item.data(), site)
        );
  ...
  const seoPage = useMemo(() => {
    ...
    return {
      ...seoPages.press,
      path: detailPath,
      title: `${article.title} | 고덕 수자인 하우스디 언론보도`,
      description:
        getArticleSummary(article) ||
        "고덕 수자인 하우스디 언론보도 상세 페이지입니다.",
      image,
      ogType: "article",
      article: {
